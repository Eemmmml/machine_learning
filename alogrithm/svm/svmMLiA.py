...
def smoP(dataMatIn, classLabels, C, toler, maxIter, kTup=("lin", 0)):  # full Platt SMO
    oS = optStruct(
        np.asmatrix(dataMatIn), np.asmatrix(classLabels).transpose(), C, toler, kTup
    )
    iter = 0
    entireSet = True
    alphaPairsChanged = 0
...
#     datMat = asmatrix(dataArr)
#     labelMat = asmatrix(labelArr).transpose()
...
#         self.alphas = asmatrix(zeros((self.m, 1)))
#         self.b = 0
#         self.eCache = asmatrix(zeros((self.m, 2)))  # first column is valid flag
...
#     oS = optStructK(asmatrix(dataMatIn), asmatrix(classLabels).transpose(), C, toler)
#     iter = 0
#     entireSet = True
#     alphaPairsChanged = 0
#     while (iter < maxIter) and ((alphaPairsChanged > 0) or (entireSet)):
