...
def createTree(dataSet, minSup=1):  # create FP-tree from dataset but don't mine
    ...
    for tranSet, count in dataSet.items():  # go through dataset 2nd time
        ...
        if len(localD) > 0:
            ...
            updateTree(
                orderedItems, retTree, headerTable, count
...
def mineTree(inTree, headerTable, minSup, preFix, freqItemList):
    bigL = [
        v[0] for v in sorted(headerTable.items(), key=lambda p: p[0])
    ]  # (sort header table)
    for basePat in bigL:  # start from bottom of header table
        ...
        if myCondTree != None:  # 3. mine cond. FP-tree
...
if __name__ == "__main__":
    ...
    data_set = createInitSet(data_set)
    fp_tree, header_table = createTree(data_set, minSup=3)
    if fp_tree != None:
        fp_tree.disp()
...
#     urlsRemoved = re.sub(
#         "(http:[/][/]|www.)([a-z]|[A-Z]|[0-9]|[/.]|[~])*", "", bigString
#     )
